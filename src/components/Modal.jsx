...
export default function Modal(props) {
  ...
  useEffect(() => {
    setInput({
      ...
      EndDate: new Date(el?.EndDate ?? null).toISOString().split("T")[0],
      status: el?.status ?? "",
    });
  }, [el?.id]);
  ...
  const hdlSubmit = async (e) => {
    try {
      ...
      const output1 = { ...input, EndDate: new Date(input.EndDate) };
      const token = localStorage.getItem("token");
      ...
      <div className="modal-box">
        <form className="flex flex-col border rounded w-5/6 mx-auto p-4 gap-4" onSubmit={hdlSubmit}>
          ...
          <label className="form-control w-full max-w-[220px] ">
            ...
            <input type="date1" name="EndDate" value={input.EndDate} onChange={hdlChange} />
          </label>
